...
class DinoV3Model(nn.Module):
    ...
    #def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #return logits
    def forward(self, x: torch.Tensor, return_protocol: bool = False):
        # 1. 正常提取用于分类的池化特征 [B, D]
        pooled_feats = self._extract_feats(
            self.backbone, 
            x, 
            pool_type=self.pool_type, 
            custom_token_indices=self.custom_token_indices
        )
        pooled_feats = F.normalize(pooled_feats, dim=-1)
        logits = self.fc(pooled_feats)

        # 2. 如果不需要协议输出，保持原样（兼容原版测试代码）
        if not return_protocol:
            return logits

        # 3. 安全提取全量 Token 序列
        raw_feats = self.backbone.forward_features(x)
        token_sequence = None
        
        if isinstance(raw_feats, dict):
            # 严格限制：只接受真正的 token tensor (ndim == 3)
            for k in ('x', 'tokens', 'last_hidden_state'):
                if k in raw_feats and raw_feats[k] is not None and raw_feats[k].ndim == 3:
                    token_sequence = raw_feats[k]
                    break
        elif raw_feats.ndim == 3:
            token_sequence = raw_feats

        if token_sequence is None:
            raise RuntimeError("Failed to extract token-level sequence [B, N, D] from backbone.forward_features().")
