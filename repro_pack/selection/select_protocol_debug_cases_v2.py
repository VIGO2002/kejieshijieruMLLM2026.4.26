...
def diverse_select(rows, n, seed=42):
    ...
    for r in rows:
        key = (r.get("dataset", ""), r.get("source_type", ""), r.get("status", ""))
...
def main():
    parser = argparse.ArgumentParser(description="扩到 12–18 张协议调试集")
    ...
    parser.add_argument("--output_dir", type=str, default="protocol_debug_set_v2")
    ...
    for r in selected:
        ...
        if not os.path.isdir(case_dir):
            missing_case_dirs.append(case_name)
            continue
    ...
    if missing_case_dirs:
        print(f"[!] 有 {len(missing_case_dirs)} 个 case 在 exports_dir 中缺失")
